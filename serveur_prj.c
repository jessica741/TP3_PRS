...
int main (int argc, char *argv[]) {
  ...
  socklen_t client_size_1;
  
  int err= recvfrom(desc2,buffer,sizeof(buffer), 0 , (struct sockaddr*)&adresse,&client_size_1);
  if (  err ==-1 )
  {
    printf("pb\n");
  }
  ...
  {
    printf("ugrek\n");
    sendto( desc2, "SYN-ACK",3, 0, (struct sockaddr*) &adresse, (socklen_t)sizeof(adresse)  );
  }
  //recvfrom(desc2,buffer,sizeof(buffer), 0 , (struct sockaddr*)&adresse, &client_size_1);
  if (buffer == "ACK" ) 
  {
    printf("SYN OK !!\n");
  }
  ...
  while (1) {
    ...
    //{
      ...
      int msgSize= recvfrom(desc2,buffer,sizeof(buffer), 0 , (struct sockaddr*)&adresse, &client_size);
      printf("msg size : %d !\n",msgSize);
      if( msgSize <= 0 )
      {
